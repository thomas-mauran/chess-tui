...
// This renders a popup for the multiplayer hosting / joining popup
pub fn render_join_prompt(frame: &mut Frame) {
    let block: Block<'_> = Block::default()
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded)
        .padding(Padding::horizontal(1))
    ...
    let text = vec![
        Line::from(""),
        Line::from("-- Enter the address:port of the game ? --").alignment(Alignment::Center),
        Line::from(""),
    ];
    ...
    let inner_popup_layout_vertical = Layout::default()
        ...
        let hosting = Paragraph::new(
            vec!["Enter the address:port".to_string()]
                .into_iter()
                .map(Line::from) // Convert each string into a Line
                .collect::<Vec<_>>(),
        )
        .block(Block::default())
        .alignment(Alignment::Center);

    frame.render_widget(hosting, inner_popup_layout_vertical[1]);
}


// This renders a popup for a promotion
pub fn render_wait_for_other_player(frame: &mut Frame) {
    let block = Block::default()
        .title("Waiting ...")
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded)
